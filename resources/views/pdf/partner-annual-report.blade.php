...
<body>
    <div class="pdf-report">
        <div class="header">
            ...
            <div class="title" style="font-weight:bold;color:#1976d2;background:#f0f4fa;padding:10px 24px;border-radius:30px;">{{ $partner->name }}</div>
        </div>

        <div class="info-box">
            <p>تاريخ الإصدار: {{ \Carbon\Carbon::now()->locale('ar')->translatedFormat('d F Y') }}</p>
            <p>المبلغ الحالي خلال هذه الفترة: {{ $partner->currentBalancefromDateToDate($fromDate, $toDate) }} جنيه</p>
            <p>المبلغ الإبتدائي: {{ $partner->initial_balance }} جنيه</p>
            <p>المبلغ الحالي بعد حساب الأرباح: {{ $partner->currentBalancefromDateToDate($fromDate, $toDate) + $partner->profitShares?->sum('profit_share') ?? 0}} جنيه</p>
        ...
        <div class="footer-summary">
            ...
            <div>
                الربح الشهري: {{ $partner->profitShares?->sum('profit_share') ?? 0}} جنيه
            </div>
        </div>
